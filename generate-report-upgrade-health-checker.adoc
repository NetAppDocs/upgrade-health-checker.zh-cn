...
** 通过 HTTPS 连接到遥测端点（+https://support.netapp.com+ 和 +https://api.activeiq.netapp.com+）
** 通过 HTTPS (443) 连接到自动更新端点（+https://api.uhc.netapp.com+、+https://gql.aiq.netapp.com+ 和 +https://api.activeiq.netapp.com+）
** 至少 4 GB 的可用 `/tmp`空间
...
. 在 Web 浏览器、Microsoft Excel 或 JSON 查看器中打开报告文件（已命名 `uhc_<cluster-name>_<YYYYMMDDHHMMSS>`）以查看报告。如果您在远程主机上运行它，请先将报告文件下载到计算机上，在那里您可以在 Web 浏览器、Excel 或 JSON 查看器中查看报告。
+
日志路径和报告路径如下：
...
.下一步
生成升级计划和集群报告后，可以查看 link:dashboard-upgrade-health-checker.html["升级运行状况检查器仪表板"] 以了解在运行期间评估的集群运行状况的详细信息。
