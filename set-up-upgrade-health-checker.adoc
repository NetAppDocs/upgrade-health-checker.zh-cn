...
* 托管要求：将 VM 放置在具有集群连接和互联网访问的位置，以允许自动工具更新。机器应通过 HTTPS (443) 访问以下端点以执行自动更新：
+
...
此外，请确保虚拟机可以连接到遥测端点 (https://support.netapp.com/[] 和 https://api.activeiq.netapp.com[]）通过 HTTPS 以启用 NetApp 接收有关升级计划的 AutoSupport 信息。
